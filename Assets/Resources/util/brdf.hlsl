...
{
    attenuation = GetAlbedoColor(hit.material, hit.uv) + hit.material.emission.rgb;

    ray.direction = normalize(ray.direction + RandomSphere(rngSeed));
...
bool ScatterMetal(RayHit hit, inout Ray ray, inout float3 attenuation, inout uint rngSeed)
{
    ...
    ray.direction = normalize(reflected + roughness * RandomSphere(rngSeed));
    attenuation = GetAlbedoColor(hit.material, hit.uv) + hit.material.emission.rgb;
